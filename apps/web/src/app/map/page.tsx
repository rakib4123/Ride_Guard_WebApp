...
export default function MapPage() {
  ...
  return (
    <MapScreen
      map={<PointPickerMap value={loc} onChange={setLoc} level={result?.advisoryLevel} hotspots={hotspots} zoomControl={false} />}
      top={
        <div className="rounded-2xl border border-line bg-white/95 px-2 py-1 shadow-soft backdrop-blur">
      ...
      sheet={
        <>
          <div className="flex items-center justify-between gap-2">
            <p className="min-w-0 truncate text-sm font-medium text-text">{locName}</p>
            {result && <AdvisoryBadge level={result.advisoryLevel} />}
          </div>
      ...
      sheetMore={
        <>
          ...
          <p className="text-xs text-muted">
            Tap or drag the pin to check the risk anywhere. The spatial layer blends your current
            trip settings with this location’s historical incident density.
          </p>
        </>
      }
    />
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-panel2 py-2">
      <div className="font-mono tabular text-base text-text">{value}</div>
